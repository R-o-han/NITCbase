    ...
    */
    ...
    for (int i = 0; i < RELCAT_NO_ATTRS; i++)
    {
        ...
        if (i < RELCAT_NO_ATTRS - 1)
            attrCacheEntryRelTemp->next = new AttrCacheEntry; // here we check if its the last iteration or not if it is we go to else scope and temp->next = nullptr
        else
    ...
    for (int i = startAttrIndex; i <= lastAttrIndex; i++)
    {
        ...
        if (i < lastAttrIndex)
            attrCacheEntryAttrTemp->next = new AttrCacheEntry;
        else
...
OpenRelTable::~OpenRelTable()
{
    ...
    {
        if (RelCacheTable::relCache[relCacheIdx] != nullptr)
            delete RelCacheTable::relCache[relCacheIdx];
    }
    ...
    for (int attrCacheIdx = 0; attrCacheIdx < MAX_OPEN; attrCacheIdx++)
        freeAttrCacheEntry(AttrCacheTable::attrCache[attrCacheIdx]);
}